    ...
    protected abstract class AbstractUnsafe implements Unsafe {
        ...
        public final void register(EventLoop eventLoop, final ChannelPromise promise) {
            ...
            if (eventLoop.inEventLoop()) {
                ...
                try {
                    //SingleThreadEventExecutor  这里调用的execute是
                    // SingleThreadEventExecutor类的方法，进入查看
                    //里面做了很多事情
        ...
        private void register0(ChannelPromise promise) {
            try {
                ...
                // 这里是到子类 AbstractNioChannel 执行 doRegister()的方法逻辑
                //这里才是NioServerSocketChannel 注册到selector上的实现，底层其实是调用
                //了Java Nio中的api进行注册的
                //调用NioServerSocketChannel 通过反射创建出来nio底层channel的register方法  选择器看不同操作系统
                //AbstractNioChannel = doRegister()方法是 AbstractNioChannel 类的实现
                doRegister();
                ...
                //这里面会去调用pipeline中的所有handler,
                // 同时服务端接收的客户端新的NioSocketChannel也是在这里注册到对应的Selector上的
