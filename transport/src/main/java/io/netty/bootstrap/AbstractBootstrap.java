...
public abstract class AbstractBootstrap<B extends AbstractBootstrap<B, C>, C extends Channel> implements Cloneable {
    ...
    //这个就是入口处的 bossGroup  //两组EventLoopGroup 这组是用于接受客户端连接的  NioEventLoopGroup 包含属性如下
    ...
    //channelFactory = ReflectiveChannelFactory(),通过构造器反射new实例，
    // 这里的构造器对应的class是NioServerSocketChannel
    ...
    final ChannelFuture initAndRegister() {
        ...
        try {
            //这里终于调用newChannel方法了，这里b.channel()初始化的ReflectiveChannelFactory 用于创建新的channel实例对象
            //channel 对象是NioServerSocketChannel。新建的通道用于绑定对应的EventLoop,就是对应的selector, EventLoop中聚合了 Selector
            //这里是服务端用于接收客户端连接的channel
            //调用的是ReflectiveChannelFactory.newChannel()方法
            channel = channelFactory.newChannel();
            //做一些channel的初始化工作，比如建立通道
            init(channel);
        } catch (Throwable t) {
            if (channel != null) {
        ...
        //EventLoopGroup bossGroup：group()返回的就是主线程对应的线程池
        EventLoopGroup eventLoopGroup = config().group();
        //用这个事件循环注册一个通道 一旦注册完成，返回的ChannelFuture将得到通知。
        //这里是selector 注册channel
