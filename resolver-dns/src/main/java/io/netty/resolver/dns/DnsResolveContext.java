...
abstract class DnsResolveContext<T> {
    ...
    private void finishResolve(Promise<List<T>> promise, Throwable cause) {
        ...
        // 如果completeEarly是正确的，我们仍然希望继续处理查询，以确保我们仍然放置所有内容
        // 最终在缓存中。
        if (!completeEarly && !queriesInProgress.isEmpty()) {
            // If there are queries in progress, we should cancel it because we already finished the resolution.
            // 如果有正在进行的查询，我们应该取消它，因为我们已经完成了解决。
        ...
        if (finalResult != null) {// 有异常 说明这里为null
            if (!promise.isDone()) {
                // Found at least one resolved record.  找到至少一个已解决的记录。
        ...
        // No resolved address found. 没有找到解析的地址。
